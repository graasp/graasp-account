...
describe('Change username', () => {
  ...
  it('Username field cannot be empty', () => {
    ...
    cy.get('input[name=username]').clear();
    cy.get(`#${PERSONAL_INFO_EDIT_SAVE_BUTTON_ID}`).should('be.disabled');
  });
  ...
  it('Valid username can be saved', () => {
    ...
    changeUsername(validUsername);
    cy.get(`#${PERSONAL_INFO_EDIT_SAVE_BUTTON_ID}`).should('not.be.disabled');

    cy.get(`#${PERSONAL_INFO_EDIT_SAVE_BUTTON_ID}`).click();
  ...
  it('Should not update the user name if canceling edit', () => {
    changeUsername('validUsername');
    cy.get(`#${PERSONAL_INFO_EDIT_CANCEL_BUTTON_ID}`).click();
